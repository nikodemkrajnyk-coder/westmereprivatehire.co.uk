...
router.post('/bookings', (req, res) => {
  ...
  /* ONE AUTHORITY DECIDES (server/calendar-sync.js). A booking created as an
     enquiry does NOT go on the calendar — it goes on when it is confirmed. A
     staff booking entered as already-confirmed goes on immediately. Either way
     this path no longer has an opinion of its own. */
...
router.patch('/bookings/:id', async (req, res) => {
  ...
  if (updated) {
    ...
    /* ── Operator's shared calendar ───────────────────────────────────────
       This is where a booking usually BECOMES confirmed — the owner's Confirm
       action — and where it is cancelled. Both, and a plain edit, are the same
       question: does the calendar match the booking. One call asks it
       (server/calendar-sync.js). The three branches this replaces created an
       event for any non-cancelled booking, which is how enquiries reached the
       calendar. */
    calendarSync.syncBookingSoon(updated.id);
...
router.post('/bookings/:id/mark-paid', (req, res) => {
  ...
  if (wasUnsettled) {
    // Settling moves it to confirmed — onto the calendar if it is not there yet.
    calendarSync.syncBookingSoon(id);
