...
// ── Per-driver Google Calendar integration ───────────────────────────────
// Each driver gets their own token row: provider = 'gcal_driver_<id>'
// This avoids schema changes — the existing integrations table + UNIQUE(provider)
// constraint works perfectly since each driver gets a unique provider key.

const DRIVER_CAL_SCOPE = [
  'https://www.googleapis.com/auth/calendar.events',
  'https://www.googleapis.com/auth/userinfo.email',
  'openid'
].join(' ');

function driverProvider(driverId) {
  return 'gcal_driver_' + driverId;
}

function buildDriverAuthUrl(driverId, state) {
  const params = new URLSearchParams({
    client_id: CLIENT_ID,
    redirect_uri: REDIRECT_URI,
    response_type: 'code',
    scope: DRIVER_CAL_SCOPE,
    access_type: 'offline',
    prompt: 'consent',
    state: state || ''
  });
...
function loadDriverTokens(driverId) {
  const db = getDb();
  return db.prepare('SELECT * FROM integrations WHERE provider = ?').get(driverProvider(driverId)) || null;
}

function saveDriverTokens(driverId, payload) {
  const db = getDb();
  const provider = driverProvider(driverId);
  const existing = loadDriverTokens(driverId);
  const now = Math.floor(Date.now() / 1000);
  const expires_at = payload.expires_in ? now + payload.expires_in - 60 : (existing && existing.expires_at) || null;
...
function clearDriverTokens(driverId) {
  const db = getDb();
  db.prepare('DELETE FROM integrations WHERE provider = ?').run(driverProvider(driverId));
}

function getDriverStatus(driverId) {
  const t = loadDriverTokens(driverId);
  return {
    configured: isConfigured(),
    connected: !!(t && t.refresh_token),
    email: t ? t.account_email : null
...
async function getDriverAccessToken(driverId) {
  const t = loadDriverTokens(driverId);
  if (!t || !t.refresh_token) return null;
  const now = Math.floor(Date.now() / 1000);
  if (t.access_token && t.expires_at && t.expires_at > now) return t.access_token;
...
// Build a calendar event body for a booking (driver-facing — simpler summary)
function bookingToDriverEvent(booking) {
  const durationMin = booking.duration_min || 90;
  const tz = 'Europe/London';
  let startIso, endIso;
  ...
  if (booking.time && /^\d{2}:\d{2}$/.test(booking.time) && booking.date) {
    ...
    const pad = n => String(n).padStart(2, '0');
    endIso = `${end.getUTCFullYear()}-${pad(end.getUTCMonth() + 1)}-${pad(end.getUTCDate())}T${pad(end.getUTCHours())}:${pad(end.getUTCMinutes())}:00`;
    const name = booking.customer_name || booking.passenger_name || 'Passenger';
    const lines = [
      `Passenger: ${name}`,
      booking.customer_phone || booking.passenger_phone ? `Phone: ${booking.customer_phone || booking.passenger_phone}` : null,
    ...
    return {
      summary: `WPH ${booking.ref || ''} — ${booking.pickup || ''} → ${booking.destination || ''}`,
      location: booking.pickup || '',
      description: lines.join('\n'),
      start: { dateTime: startIso, timeZone: tz },
      end:   { dateTime: endIso,   timeZone: tz },
      extendedProperties: { private: { wph_booking_id: String(booking.id || ''), wph_ref: booking.ref || '' } }
    };
  }
  return {
    summary: `WPH ${booking.ref || ''} — ${booking.pickup || ''}`,
    start: { date: booking.date },
    end:   { date: booking.date },
    extendedProperties: { private: { wph_booking_id: String(booking.id || ''), wph_ref: booking.ref || '' } }
  };
...
async function createDriverCalendarEvent(driverId, booking) {
  if (!isConfigured()) return null;
  const token = await getDriverAccessToken(driverId).catch(() => null);
  if (!token) return null;
  try {
    const r = await fetch(`${API_BASE}/calendars/primary/events`, {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
      body: JSON.stringify(bookingToDriverEvent(booking))
    });
    const d = await r.json();
    if (!r.ok) { console.error('[GCAL] driver createEvent HTTP', r.status, d && d.error && d.error.message); return null; }
    return d.id || null;
  } catch (e) {
    console.error('[GCAL] driver createEvent failed:', e.message);
    return null;
  }
}

async function deleteDriverCalendarEvent(driverId, eventId) {
  if (!isConfigured() || !eventId) return false;
  const token = await getDriverAccessToken(driverId).catch(() => null);
  if (!token) return false;
  try {
    const r = await fetch(`${API_BASE}/calendars/primary/events/${encodeURIComponent(eventId)}`, {
      method: 'DELETE',
      headers: { Authorization: 'Bearer ' + token }
    });
    return r.ok || r.status === 404 || r.status === 410;
  } catch (e) {
    console.error('[GCAL] driver deleteEvent failed:', e.message);
    return false;
  }
}
...
module.exports = {
  ...
  createDriverCalendarEvent,
  deleteDriverCalendarEvent,
  createEvent,
  updateEvent,
  deleteEvent,
  pullChanges,
